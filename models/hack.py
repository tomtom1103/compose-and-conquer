...
class CheckpointCopyCallback(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        for ckpt_path in all_checkpoints:
            ...
            dst = os.path.join(self.dst_path, 'ckpt', run_name, os.path.basename(ckpt_path))
            os.makedirs(os.path.dirname(dst), exist_ok=True)
            if ckpt_path not in self.copied_files:
                shutil.copy2(ckpt_path, dst)
                self.copied_files.add(ckpt_path)
...
# from https://github.com/basujindal/stable-diffusion/blob/main/optimizedSD/splitAttention.py
...
def sanity_check(batch):
    ...
    tr(batch['bg_depth'][0][0]).save('bg_depth.jpg')
    tr(batch['fg_depth'][0][0]).save('fg_depth.jpg')
    tr(batch['bg_color'][0][0]).save('bg_color.jpg')
    tr(batch['fg_color'][0][0]).save('fg_color.jpg')