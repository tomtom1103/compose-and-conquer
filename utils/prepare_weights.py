...
def init_local(sd_weights_path, config_path, output_path):
    ...
    scratch_dict = model.state_dict() #랜덤init 된 내 모델
    target_dict = {}
    for sk in scratch_dict.keys(): #랜덤init 된 내 모델의 파라미터명
...
def init_local_fromuni(sd_weights_path, config_path, output_path):
    pretrained_weights = torch.load(sd_weights_path)
    if 'state_dict' in pretrained_weights:
        pretrained_weights = pretrained_weights['state_dict']
    model = create_model(config_path=config_path)
...
if __name__ == '__main__':
    mode = sys.argv[1]
    assert mode in ['init_local', 'init_global', 'integrate', 'init_local_fromuni']
    if mode == 'init_local':
    ...
    elif mode == 'init_local_fromuni':
        assert len(sys.argv) == 5
        init_local_fromuni(sys.argv[2], sys.argv[3], sys.argv[4])
    else:
